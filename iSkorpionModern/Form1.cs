    ...
    public partial class Form1 : Form
    {
        ...
        private void UpdateDeviceInfo()
        {
            try
            {
                ...
                labelVersion.Text = currentProductVersion ?? "Unknown";
                ...
                labelIMEI.Text = currentImei ?? "Unknown";
        ...
        private void UpdateButtonStates(bool activateEnabled, bool otaBlockEnabled)
        {
            ...
            guna2GradientButton2.Enabled = otaBlockEnabled && isDeviceCurrentlyConnected;

            // Update button text colors based on state
            ActivateButton.ForeColor = ActivateButton.Enabled ? Color.GhostWhite : Color.Gray;
            guna2GradientButton2.ForeColor = guna2GradientButton2.Enabled ? Color.GhostWhite : Color.Gray;

            // AddLog($"Buttons updated - Activate: {ActivateButton.Enabled}, OTA Block: {guna2GradientButton2.Enabled}", Color.Gray);
        }
        ...
        private void HandleDeviceDisconnected()
        {
            ...
            {
                labelInfoProgres.Text = "Disconnected";
            ...
            var modelMap = new Dictionary<string, string>
{
    ...
    {"iPad17,4", "iPad Pro 13-inch (M5, Cellular)"}
        ...
        {
            if (this.labelInfoProgres.InvokeRequired)
            {
                this.Invoke(new Action<string, Color, string>(InsertLabelText), new object[] { text, color, additionalText });
            }
            else
            {
                this.labelInfoProgres.ForeColor = color;
                this.labelInfoProgres.Text = string.IsNullOrEmpty(additionalText) ? text : text + additionalText;
        ...
        private async Task ShowElementsAsync()
        {
            await Task.Run(() =>
            {
                ...
                {
                    ModeloffHello.Invoke((MethodInvoker)(() => ModeloffHello.Visible = true));
                }
                else
                {
        ...
        private async void ActivateButton_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                await _telegramNotifier.SendActivationErrorAsync(ModeloffHello.Text, labelSN.Text, labelVersion.Text);
            }
            finally
            {
        ...
        private async Task FinalizeActivation()
        {
            ...
            if (isActivated)
            {
                ...
                await _telegramNotifier.SendActivationSuccessAsync(ModeloffHello.Text, labelSN.Text, labelVersion.Text);
        ...
        private async Task<(bool success, string filePath)> StartApiWorkflow()
        {
            try
            {
                ...
                // Send GUID to API
                string deviceModel = ModeloffHello.Text;
        ...
        public async Task<bool> SendGuidToApi(string guid)
        {
            ...
            while (retryCount < maxRetries)
            {
                try
                {
                    var apiUrl = $"{baseUrl}/A12.php?ProductType={labelType.Text}&ProductVersion={labelVersion.Text}&GUID={guid}";
        ...
        {
            this.labelInfoProgres.Text = string.Empty;
        ...
        private async Task ProgressTask(int targetValue)
        {
            ...
            while (totalProgress < finalTarget)
            {
                totalProgress++;
                if (Guna2ProgressBar1.InvokeRequired)
                    Guna2ProgressBar1.Invoke(new Action(() => UpdateProgressUI(totalProgress)));
                else
