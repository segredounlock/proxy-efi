...
function cmd_broadcast($chat_id, $full_text) {
    ...
    // PROTEÇÃO ANTI-DUPLICAÇÃO: Verificar se a mensagem é idêntica à última broadcast
    ...
    if (file_exists($last_broadcast_file)) {
        ...
        if ($last_broadcast === $message) {
            ...
            $msg .= "💡 <b>Dica:</b> Não copie e cole mensagens do sistema (como confirmações de gifts) como broadcast.";
            send_message($chat_id, $msg);
    ...
    // Salvar mensagem atual para comparação futura
    ...
    foreach ($users as $u) {
        ...
        if (!empty($u['chat_id'])) {
            ...
            usleep(100000); // 0.1 segundo (mais rápido)
        }
    }
